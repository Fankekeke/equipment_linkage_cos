...
export default {
  ...
  methods: {
    audit (id, status) {
      this.$get('/cos/event-info/setupOpen', {deviceId: id, openFlag: status}).then((r) => {
        this.$message.success('修改场景事件成功')
        this.search()
      })
    },
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/event-info/' + ids).then(() => {
